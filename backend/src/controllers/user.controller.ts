  ...
  if (checkifUserExists) {
    return res
      ...
      .json(
        new ApiError(
          400,
          "User already exists , please try with new username or email"
        )
      );
  }
...
const generateAccessToken = async (userId) => {
  try {
    ...
    throw new ApiError(
      503,
      "unable to generate tokens right now ! please try again"
    );
  }
};
...
const userLogin = AsyncHandler(async (req, res) => {
  ...
  return res
    ...
    .json(new ApiResponse(200, userLogged, "User logged in successfully"));
});
...
const EditUserProfile = async (req, res) => {
  ...
  const UpdateUserProfile = userSchema.parse({
    FullName,
    Bio,
    location,
    webLink,
  ...
  const updatedUser = await User.findByIdAndUpdate(
    ...
    UpdateUserProfile,
    { new: true }
  ).select("-password -email -accessToken");
  if (!updatedUser) {
...
const changeUsername = async (req, res) => {
  ...
  const updatedUser = await User.findByIdAndUpdate(user._id, UpdateUsername, {
    new: true,
  }).select("-password -email -accessToken");
  if (!updatedUser) {
...
const fetchUserProfile = async (req, res) => {
  try {
    ...
    const userprofile = await User.findOne({ username: username })
      .select("-password -accessToken -subId")
      .lean();
    if (!userprofile) {
    ...
    return res
      .status(500)
      .json(new ApiError(500, "Failed to fetch user profile"));
  }
};
  ...
  try {
    ...
    const newUser = await User.create({
      ...
      username: userInfo.nickname
        ? userInfo.nickname.slice(0, 15)
        : userInfo.name.replace(/\s/g, "").slice(0, 15),
      name: userInfo.name,
    ...
    if (!newUser) {
      return res
        .status(500)
        .json(new ApiError(500, "Unable to create a new user"));
    }
